...
function App() {
  ...
  const [applyFatherOcc, setApplyFatherOcc] = useState('');
  const [applyMotherOcc, setApplyMotherOcc] = useState('');
  ...
  const handleApply = async (e) => {
    ...
    try {
      ...
      formData.append('family_annual_income', applyIncome);
      formData.append('father_occupation', applyFatherOcc);
      formData.append('mother_occupation', applyMotherOcc);
      formData.append('phone_number', applyPhone);
            ...
            <div className="glass-card auth-card" style={{ maxWidth: '800px', width: '90%' }}>
              ...
              <form onSubmit={handleApply}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
                  ...
                  <div className="form-group">
                    <label className="form-label">Family Annual Income (INR) *</label>
                    <input 
                      type="number" 
                      step="0.01"
                      className="form-control" 
                  ...
                  <div className="form-group">
                    ...
                    <input 
                      ...
                      className="form-control" 
                      required 
                      value={applyFatherOcc} 
                      onChange={e => setApplyFatherOcc(e.target.value)} 
                      placeholder="e.g. Business / Farmer"
                    />
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    <input 
                      ...
                      className="form-control" 
                      required 
                      value={applyMotherOcc} 
                      onChange={e => setApplyMotherOcc(e.target.value)} 
...
function ApplicationsPanel() {
  ...
  const handleQuickCall = async (appId) => {
    try {
      setCalling(true);
      const updatedApp = await api.batches.callStudent(appId);
      setSelectedApp(updatedApp);
      alert("Call initiated successfully via SnapServe AI! The agent is dialing the student.");
      
      // Start polling for status changes
      let pollCount = 0;
      const interval = setInterval(async () => {
        pollCount += 1;
        if (pollCount > 15) { // Stop polling after 45 seconds max
          clearInterval(interval);
          setCalling(false);
          return;
        }
        try {
          const freshApp = await api.batches.getApplication(appId);
          if (freshApp.call_status !== 'calling') {
            setSelectedApp(freshApp);
            clearInterval(interval);
            setCalling(false);
            fetchApps(); // Update main list too
          }
        } catch (err) {
          console.error("Error polling application call status:", err);
          clearInterval(interval);
          setCalling(false);
        }
      }, 3000);
        ...
        <div className="modal-backdrop" style={{
          ...
          <div className="glass-card" style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '24px' }}>
              ...
              <div>
                <strong style={{ display: 'block', fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Phone Number</strong>
                <span>{selectedApp.phone_number || 'N/A'}</span>
              </div>
              <div>
                <strong style={{ display: 'block', fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Family Annual Income</strong>
                <span>{selectedApp.family_annual_income ? `₹${selectedApp.family_annual_income.toLocaleString()}` : 'N/A'}</span>
              </div>
              <div>
                <strong style={{ display: 'block', fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Father's Occupation</strong>
                <span>{selectedApp.father_occupation || 'N/A'}</span>
              </div>
              <div>
                <strong style={{ display: 'block', fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Mother's Occupation</strong>
                <span>{selectedApp.mother_occupation || 'N/A'}</span>
              </div>
              <div>
              ...
              <div>
                <strong style={{ display: 'block', fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Call Status</strong>
                <span className="badge" style={
                  selectedApp.call_status === 'success' ? { background: 'rgba(16, 185, 129, 0.15)', color: 'var(--accent-emerald)', textTransform: 'capitalize' } :
                  selectedApp.call_status === 'calling' ? { background: 'rgba(59, 130, 246, 0.15)', color: 'var(--accent-blue)', textTransform: 'capitalize' } :
                  selectedApp.call_status === 'failed' ? { background: 'rgba(244, 63, 94, 0.15)', color: 'var(--accent-rose)', textTransform: 'capitalize' } :
                  { background: 'rgba(148, 163, 184, 0.15)', color: 'var(--text-secondary)', textTransform: 'capitalize' }
                }>
                  {selectedApp.call_status || 'pending'}
                </span>
              </div>
            </div>
            ...
            {/* Call Verification Details Section */}
            <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '20px', marginTop: '20px' }}>
              <div className="flex-between" style={{ marginBottom: '16px' }}>
                <h4 style={{ margin: 0 }}>Admission Verification Call (SnapServe AI)</h4>
                {selectedApp.status === 'approved' && (
                  <button 
                    className="btn btn-primary"
                    style={{ padding: '6px 16px', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '6px' }}
                    onClick={() => handleQuickCall(selectedApp.id)}
                    disabled={calling || selectedApp.call_status === 'calling'}
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                    {calling || selectedApp.call_status === 'calling' ? 'Calling...' : 'Quick Call'}
                  </button>
                )}
              </div>

              {selectedApp.call_status === 'calling' && (
                <div style={{ background: 'rgba(59, 130, 246, 0.05)', border: '1px solid rgba(59, 130, 246, 0.2)', padding: '16px', borderRadius: '8px', textAlign: 'center', color: 'var(--text-secondary)' }}>
                  <div className="spinner" style={{ margin: '0 auto 12px auto', width: '24px', height: '24px', border: '3px solid rgba(59, 130, 246, 0.2)', borderTopColor: 'var(--accent-blue)', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>
                  <style>{`@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }`}</style>
                  Calling candidate... Please wait while SnapServe conducts the admissions briefing and verification.
                </div>
              )}

              {selectedApp.call_status === 'success' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-color)', padding: '12px', borderRadius: '8px' }}>
                    <strong style={{ display: 'block', fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '8px' }}>Call Recording</strong>
                    <audio controls src={selectedApp.call_recording_url} style={{ width: '100%' }} />
                  </div>
                  
                  <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-color)', padding: '16px', borderRadius: '8px' }}>
                    <strong style={{ display: 'block', fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '10px' }}>Call Transcript</strong>
                    <div style={{ maxHeight: '200px', overflowY: 'auto', padding: '12px', background: 'rgba(0,0,0,0.2)', borderRadius: '6px', fontSize: '0.9rem', lineHeight: '1.6', whiteSpace: 'pre-line', border: '1px solid rgba(255,255,255,0.05)', fontFamily: 'system-ui, sans-serif' }}>
                      {selectedApp.call_transcript}
                    </div>
                  </div>
                </div>
              )}

              {selectedApp.call_status === 'failed' && (
                <div style={{ background: 'rgba(244, 63, 94, 0.05)', border: '1px solid rgba(244, 63, 94, 0.2)', padding: '16px', borderRadius: '8px', color: 'var(--accent-rose)' }}>
                  Verification call failed or was disconnected. Please click "Quick Call" to try again.
                </div>
              )}

              {(selectedApp.call_status === 'pending' || !selectedApp.call_status) && (
                <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-color)', padding: '16px', borderRadius: '8px', textAlign: 'center', color: 'var(--text-secondary)' }}>
                  {selectedApp.status === 'approved' ? 
                    'No verification call has been placed yet. Click "Quick Call" to initiate AI verification.' : 
                    'Verification calling is only available once the application is approved.'}
                </div>
              )}
            </div>
