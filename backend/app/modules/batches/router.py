...
async def update_application_status(
    ...
    app["id"] = str(app["_id"])
    app["batch_id"] = str(app.get("batch_id", ""))
    app["applied_at"] = app.get("applied_at") or datetime.now(timezone.utc)
    ...
    batch_id_val = app.get("batch_id")
    if isinstance(batch_id_val, ObjectId):
        batch = await db.batches.find_one({"_id": batch_id_val})
    elif ObjectId.is_valid(str(batch_id_val)):
        batch = await db.batches.find_one({"_id": ObjectId(batch_id_val)})
    else:
        batch = None
...
async def update_interview_details(
    ...
    """Area Head or Center Associate updates interview details for an approved student application and triggers invitation email."""
    if not ObjectId.is_valid(app_id):
        raise HTTPException(status_code=400, detail="Invalid application ID")
                        ...
                        if cloudinary_url and recording_url:
                            try:
                                ...
                                if audio_res.status_code == 200:
                                    ...
                                    print(f"Warning: Failed to download audio from SnapServe: {audio_res.status_code}")
                            except Exception as e:
                                print(f"Warning: Failed to upload SnapServe recording to Cloudinary: {str(e)}")
                        elif recording_url:
                            recording_url = f"/api/batches/applications/{app_id}/recording"
...
async def get_application_call_recording(
    ...
    if not call_id:
        raise HTTPException(status_code=400, detail="No call has been placed for this application.")
        
    snapserve_api_key = getattr(settings, "SNAPSERVE_API_KEY", "")
    if not snapserve_api_key:
        raise HTTPException(status_code=500, detail="SnapServe API key is not configured.")
    ...
    async with httpx.AsyncClient() as client:
        ...
        if res.status_code != 200:
            raise HTTPException(status_code=res.status_code, detail=f"Failed to fetch call details from SnapServe: {res.text}")
            
        data = res.json()
        recording_url = data.get("recordingUrl")
        if not recording_url:
            raise HTTPException(status_code=404, detail="Recording is not available for this call.")
        ...
        if audio_res.status_code != 200:
            raise HTTPException(status_code=audio_res.status_code, detail="Failed to download recording from SnapServe.")
            
        return StreamingResponse(io.BytesIO(audio_res.content), media_type="audio/mpeg")
